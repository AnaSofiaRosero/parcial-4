...
// Modal que aparece al presionar "Editar" en la tabla de empleados.
// Carga los datos del empleado seleccionado y permite modificarlos.
// Props:
//   empleado      → empleado a editar (null = modal cerrado)
//   departamentos → lista de departamentos para el selector
...
export default function EmpleadoModal({ empleado, departamentos, onClose, onSaved }: Props) {
  ...
  useEffect(() => {
    if (empleado) {
      ...
      setSalario(String(empleado.salario)); // Convertir número a string para el input
      setDepartamentoId(empleado.departamento_id);
    }
  }, [empleado]); // Se ejecuta cuando cambia la prop empleado

  // Si no hay empleado seleccionado, no renderizar el modal
  if (!empleado) return null;

  // Actualiza el empleado en Supabase con los nuevos datos
  const guardar = async () => {
    const { error } = await supabase
      ...
      .update({
        nombre,
        puesto,
        salario: Number(salario),         // Convertir de vuelta a número
        departamento_id: departamentoId,  // Foreign Key
      })
      .eq("id", empleado.id); // Solo actualiza el registro con este id
